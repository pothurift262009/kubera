...
class KiteBroker:
    ...
    def generate_session(self, request_token: str) -> str:
        data  = self.kite.generate_session(request_token, api_secret=self.config["kite"]["api_secret"])
    ...
    def place_sl_order(self, symbol: str, direction: str, qty: int, sl_price: float) -> str:
        ...
        oid = self.kite.place_order(
            ...
            trigger_price=trigger, price=limit,
            product=self.kite.PRODUCT_MIS,
            variety=self.kite.VARIETY_REGULAR,
        )
        logger.info(f"SL order: {symbol} trigger={trigger} | {oid}")
        return oid
    ...
    def _place(self, symbol: str, qty: int, tx_type) -> str:
        ...
        logger.info(f"Order placed: {tx_type} {qty} {symbol} | ID={oid}")
        return oid
