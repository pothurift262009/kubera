"""
Backtest Engine — VWAP Reversion on 1-min data
"""
...
class BacktestEngine:
    ...
    def run(self, start_date=None, end_date=None, progress_cb=None, log_cb=None) -> dict:
        ...
        if not all_dates:
            return {"error": "No trading dates in selected range."}

        log(f"Period    : {all_dates[0]} → {all_dates[-1]}  ({len(all_dates)} days)")
        log(f"Symbols   : {len(self.stock_data)}  |  Capital : ₹{self.initial_cap:,.0f}")
        log(f"Strategy  : VWAP Mean Reversion (1-min)")
        log(f"Deviation : {self.config['strategy'].get('vwap_deviation_pct',1.5)}%  |  "
            f"RSI : {self.config['strategy'].get('rsi_oversold',32)}/"
            f"{self.config['strategy'].get('rsi_overbought',68)}  |  "
            f"Max pos : {self.max_pos}")
        log("─" * 55)
        ...
        for i, date in enumerate(all_dates):
            ...
            if next_date is None or str(next_date)[:7] != month_key:
    ...
    def _run_day(self, date, capital: float, log) -> tuple:
        ...
        log(f"[{date}] {len(signals)} signal(s)")
        trades, pnl = [], 0.0
        for sig in signals:
