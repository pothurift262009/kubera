...
from backtest import run_backtest_elite_v5
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("elite_pipeline_v5.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger("Elite_NSE_Pipeline_V5")
...
def main():
    ...
    # CONFIG (V5 ALPHA STRATEGY)
    # ISSUE 2: tp_mult 2.0, sl_mult 1.0 (Asymmetric payload)
    LABEL_CONFIG = {'tp_mult': 2.0, 'sl_mult': 1.0, 'max_bars': 6}
    BT_CONFIG = {
        'cost': 0.0002, 
        'slippage': 0.0001, 
        'long_threshold': 0.55, # ISSUE 3
        'short_threshold': 0.58, # ISSUE 3
        'confidence_gap_threshold': 0.12, # ISSUE 3
        'expectancy_threshold': 0.0005, # ISSUE 1: Trade Expectancy
        'vol_filter_quantile': 0.2, # ISSUE 5
        'spread_filter_threshold': 0.0015
    }
    
    # 1. DATA SOURCE (V2 Parquet Persistence)
    ...
    # 4. LABELING (V5: Asymmetric High-Reward targets)
    df_labeled = apply_triple_barrier_elite_v5(df_combined, config=LABEL_CONFIG)
    
    # 5. MODELING (Optimized for High-Prob filters)
    ...
    # 6. HOLD-OUT ENSEMBLE EVALUATION
    ...
    # 7. ELITE BACKTESTING V5 (Expectancy & Confirmation Filtering)
    bt_df, portfolio_rets, metrics = run_backtest_elite_v5(
        df_test, probs, 
        cost=BT_CONFIG['cost'], 
        ...
        expectancy_threshold=BT_CONFIG['expectancy_threshold'],
    ...
    logger.info(f"V5 ALPHA SUMMARY: Sharpe={final_sharpe:.2f}, PF={final_pf:.2f}, Trades={num_trades}")
    logger.info("Elite Pipeline V5 Execution Finished.")

if __name__ == "__main__":
    try:
        main()
    except Exception as e:
        logger.error(f"Elite Pipeline V5 fatal error: {e}", exc_info=True)
